...
// 新增：补全 querySelector 方法（mock 实现，返回空元素或基于现有 mock）
Document.prototype.querySelector = function querySelector(selector) {
    // console.log(`对象 => Document.prototype.querySelector, 查询选择器: ${selector}`);
    // 简单 mock：返回 body 或空（可扩展为匹配逻辑）
    if (selector === 'body' || selector === '*') {
        return document.body;  // 使用现有 mock body
    }
    return null;  // 或返回新 Element 实例
};
...
// document = watch(document, "document");  // 启用 watch，现已兼容
